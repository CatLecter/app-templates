...
class UserController(Controller):
    ...
    async def get_user(self, db: PostgresDB, user_id: UUID4) -> Response[ResponseUser | dict]:
        ...
        return Response(
            content=user.json() if user else {'result': 'failed', 'detail': f'User with UUID={user_id} not found'},
    ...
    async def add_user(self, db: PostgresDB, data: User) -> Response[ResponseUser | dict]:
        ...
        return Response(
            content=user.json() if user else {'result': 'failed', 'detail': 'Failed to create user'},
